...
<body>
    ...
    <table class="table table-hover">
        ...
        <c:forEach var="atrasadas" items="${atrasadas}">
        <tbody>
            <tr>
                <td class="table-warning">${atrasadas.id}</td>
                <td class="table-warning">${atrasadas.descricao}</td>
                <td class="table-warning"> <fmt:parseDate value="${atrasadas.dataConclusao}" var="convertido" pattern="yyyy-MM-dd'T'HH:mm" />
                <fmt:formatDate value="${convertido}" pattern="dd/MM/yyyy HH:mm" />
                </td>
                <td class="table-warning">
                <input type="checkbox" name="concluido${atrasadas.id}" ${atrasadas.concluido ? 'checked' : ''} />
                </td>
                <td class="table-warning">
                <a href="/todo-app/editar/${atrasadas.id}"><button class="btn btn-secondary">Editar</button></a>
                <a href="/todo-app/excluir/${atrasadas.id}"><button class="btn btn-danger">Excluir</button></a>
